...
# -------- Block from a post (reposts/likes) --------

def _try_open_people_sheet(page, kind: str = "any") -> bool:
    # Try to open the Reposts/Likes sheet by clicking respective counters/buttons
    kinds = [kind] if kind in ("reposts", "likes") else ["reposts", "likes"]
    for k in kinds:
        selectors = []
        if k == "reposts":
            selectors = [
                'a:has-text("Reposts")',
                'div[role="button"]:has-text("Reposts")',
                'div[role="link"]:has-text("Reposts")',
                'a[href*="retweet"]',
                'a[href*="retweeted_by"]',
            ]
        else:  # likes
            selectors = [
                ...
                'div[role="link"]:has-text("Likes")',
                'a[href*="likes"]',
                'a[href*="liked_by"]',
            ]
        if _click_first(page, selectors, timeout_ms=1200):
            # Wait for the people list (UserCell) to appear
            try:
                page.locator('[data-testid="UserCell"]').first.wait_for(state="attached", timeout=4000)
                return True
            except Exception:
                # continue and try the other kind
                pass
    return False


def _collect_handles_from_people_sheet(page, limit: int = 100) -> list[str]:
    handles: list[str] = []
    seen: set[str] = set()
    stable = 0
    cells = page.locator('[data-testid="UserCell"]')
    while len(handles) < limit and stable < 5:
        try:
            count = cells.count()
        except Exception:
            count = 0
        # extract hrefs -> handles
        try:
            hrefs = page.evaluate(
                """
                () => Array.from(document.querySelectorAll('[data-testid="UserCell"] a[href^="/"]'))
        ...
        for href in hrefs:
            try:
                ...
                if not href.startswith("/"):
                    continue
                user = href.split("/")[1].strip()
                if not user or user in ("i", "home", "compose"):
                    continue
                h = f"@{user}"
                if h not in seen:
                    seen.add(h)
        ...
        if len(handles) >= limit:
            break
        # scroll down a chunk to load more
        try:
...
def handles_from_post_sync(url: str, kind: str = "any", limit: int = 100) -> list[str]:
    with sync_playwright() as p:
        # persistent context to reuse login
        if settings.user_data_dir:
            ctx = p.chromium.launch_persistent_context(
                settings.user_data_dir,
                headless=settings.headless,
        ...
        opened = _try_open_people_sheet(page, kind=kind)
        if not opened:
            # Try again with small nudge scrolls
            for _ in range(3):
                _home_scroll(page)
                page.wait_for_timeout(200)
                if _try_open_people_sheet(page, kind=kind):
                    opened = True
                    break
        if not opened:
            if browser:
                ...
                ctx.close()
            return []
        handles = _collect_handles_from_people_sheet(page, limit=limit)
        if browser:
            browser.close()
        else:
            ctx.close()
...
async def handles_from_post(url: str, kind: str = "any", limit: int = 100) -> list[str]:
    return await asyncio.to_thread(handles_from_post_sync, url, kind, limit)
